      ...
      {!selectedCategoryId ? (
        ...
        <section className="space-y-4">
          ...
          <div className="overflow-x-auto rounded-xl border border-[#333] bg-[#161616]">
            <Table className="min-w-[1320px] border-separate border-spacing-0 border-2 border-slate-300 text-[13px]">
              <TableHeader className="[&_th]:border-b [&_th]:border-r [&_th]:border-slate-300"><TableRow className="border-slate-300 hover:bg-transparent"><TableHead className="w-11 whitespace-normal px-2 text-xs leading-tight text-gray-400">S.No.</TableHead><TableHead className="min-w-48 whitespace-normal px-2 text-xs leading-tight text-gray-400">Product</TableHead><TableHead className="min-w-28 whitespace-normal px-2 text-xs leading-tight text-gray-400">Customer Price</TableHead><TableHead className="min-w-28 whitespace-normal px-2 text-xs leading-tight text-gray-400">Wholesale Price</TableHead><TableHead className="min-w-32 whitespace-normal px-2 text-xs leading-tight text-gray-400">New Customer Price</TableHead><TableHead className="min-w-32 whitespace-normal px-2 text-xs leading-tight text-gray-400">New Wholesale Price</TableHead><TableHead className="min-w-36 whitespace-normal px-2 text-xs leading-tight text-gray-400">Schedule</TableHead><TableHead className="min-w-32 whitespace-normal px-2 text-xs leading-tight text-gray-400">Stock</TableHead><TableHead className="min-w-44 whitespace-normal px-2 text-xs leading-tight text-gray-400">Pending Change</TableHead><TableHead className="min-w-28 whitespace-normal px-2 text-right text-xs leading-tight text-gray-400">Action</TableHead></TableRow></TableHeader>
              <TableBody className="[&_td]:border-b [&_td]:border-r [&_td]:border-slate-300">
                {isProductsLoading ? <TableRow className="border-[#333]"><TableCell colSpan={10} className="h-48 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-[#86efac]" /></TableCell></TableRow> : products.length === 0 ? <TableRow className="border-[#333]"><TableCell colSpan={10} className="h-48 text-center text-gray-400">No products found in this category.</TableCell></TableRow> : products.map((product, index) => {
                  ...
                  return <TableRow key={product._id} className="border-[#333] hover:bg-[#1A1A1A]/70"><TableCell className="px-2 text-xs text-gray-400">{(page - 1) * 20 + index + 1}</TableCell><TableCell className="px-2"><div className="font-medium text-white">{product.name}</div><div className="mt-0.5 text-xs text-gray-500">{product.sku || "No SKU"}</div></TableCell><TableCell className="font-medium text-white">{formatPrice(product.retailPrice)}</TableCell><TableCell className="font-medium text-white">{formatPrice(product.wholesalePrice)}</TableCell><TableCell><Input type="text" inputMode="decimal" value={draft.retailPrice} onChange={(event) => updateDraft(product._id, { retailPrice: event.target.value })} placeholder="Enter customer price" aria-label={`New customer price for ${product.name}`} className="h-8 border-[#333] bg-[#0D0D0D] px-2 text-sm text-white placeholder:text-xs placeholder:text-gray-600" /></TableCell><TableCell><Input type="text" inputMode="decimal" value={draft.wholesalePrice} onChange={(event) => updateDraft(product._id, { wholesalePrice: event.target.value })} placeholder="Enter wholesale price" aria-label={`New wholesale price for ${product.name}`} className="h-8 border-[#333] bg-[#0D0D0D] px-2 text-sm text-white placeholder:text-xs placeholder:text-gray-600" /></TableCell><TableCell><div className="space-y-1"><Select value={draft.priceChangeMode} onValueChange={(value) => updateDraft(product._id, { priceChangeMode: value as RowDraft["priceChangeMode"] })}><SelectTrigger className="h-8 border-[#333] bg-[#0D0D0D] px-2 text-sm text-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="immediate">Immediately</SelectItem><SelectItem value="schedule_24h">After 24 hours</SelectItem><SelectItem value="schedule_48h">After 48 hours</SelectItem><SelectItem value="custom">Custom date (8:00 AM IST)</SelectItem></SelectContent></Select>{draft.priceChangeMode === "custom" ? <Input type="date" value={draft.customEffectiveAt} onChange={(event) => updateDraft(product._id, { customEffectiveAt: event.target.value })} className="h-8 border-[#333] bg-[#0D0D0D] px-2 text-sm text-white" aria-label={`Custom date at 8:00 AM IST for ${product.name}`} /> : null}</div></TableCell><TableCell><div className="space-y-1"><p className="text-sm font-medium text-white">{Number(product.stock || 0).toLocaleString("en-IN")} units</p><div className="flex gap-1"><Input type="text" inputMode="numeric" pattern="[0-9]*" value={stockDraft} onChange={(event) => updateStockDraft(product._id, event.target.value)} placeholder="Qty to add" aria-label={`Add stock for ${product.name}`} className="h-8 min-w-0 border-[#333] bg-[#0D0D0D] px-2 text-sm text-white placeholder:text-xs placeholder:text-gray-600" /><Button type="button" onClick={() => void addStock(product)} disabled={savingStockProductId === product._id} className="h-8 shrink-0 bg-[#86efac] px-2 text-sm text-black hover:bg-[#86efac]/90">{savingStockProductId === product._id ? <Loader2 className="h-4 w-4 animate-spin" /> : "Add"}</Button></div></div></TableCell><TableCell>{hasPending ? <div className="space-y-1 text-xs"><Badge className="bg-amber-500/15 text-amber-300 hover:bg-amber-500/15">Pending</Badge>{product.pendingRetailPrice !== null && product.pendingRetailPrice !== undefined ? <p className="text-gray-300">Customer: {formatPrice(product.pendingRetailPrice)}</p> : null}{product.pendingWholesalePrice !== null && product.pendingWholesalePrice !== undefined ? <p className="text-gray-300">Wholesale: {formatPrice(product.pendingWholesalePrice)}</p> : null}<p className="text-gray-500">{formatIst(product.priceChangeEffectiveAt)}</p></div> : <span className="text-xs text-gray-500">No pending change</span>}</TableCell><TableCell className="text-right"><Button type="button" onClick={() => void schedulePriceChange(product)} disabled={savingProductId === product._id} className="h-8 min-w-28 px-2 text-sm text-black bg-[#86efac] hover:bg-[#86efac]/90">{savingProductId === product._id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Clock3 className="mr-2 h-4 w-4" />}Save price</Button></TableCell></TableRow>
                })}
              </TableBody>
            </Table>
          </div>
